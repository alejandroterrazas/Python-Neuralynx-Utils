...
def readEEG(filename):
  ...
  nrecs=len(eegdata)//1044
  ts = np.ndarray((nrecs,), '<Q', eegdata, 0, (1044,))

  eeg  = np.ndarray((nrecs,512), '<h', eegdata, 20, (1044,2)).flatten()
...
def returnPowerSpectrum(eegdata):
  fs = 2003
  time_step = 1./fs
  freqs = np.fft.fftfreq(fs)*fs
  ps = abs(np.fft.fft(eegdata,fs))**2
  return ps
